...
class miniESN_unstable_Cell(keras.layers.AbstractRNNCell):
    ...
    def build(self, inputs_shape):
        ...
        self.kernel_extra = self.add_weight(
            name="kernel_extra",
            shape=[self.units, self.units],
            initializer=_esn_recurrent_initializer,
            trainable=False,
    ...
    def call(self, inputs, state):
        ...
        output = tf.linalg.matmul(self.activation(output), self.kernel_extra)
...
class miniESN_stable_Cell(keras.layers.AbstractRNNCell):
    """Reduced Echo State recurrent Network (ESN) cell.
    Arguments:
        units: Positive integer, dimensionality in the reservoir.
        connectivity: Float between 0 and 1.
    ...
    def build(self, inputs_shape):
        ...
        self.kernel_linear = self.add_weight(
            name="kernel_linear",
            shape=[self.units, self.units],
            initializer=_esn_recurrent_initializer,
            trainable=False,
